...
typedef struct _redir *redir_ty;

typedef enum _quote { SingleQuote=1, DoubleQuote=2, NoQuote=3 } quote_ty;
...
typedef struct {
    _ASDL_SEQ_HEAD
    redir_ty typed_elements[1];
} asdl_redir_seq;

asdl_redir_seq *_Py_asdl_redir_seq_new(Py_ssize_t size, PyArena *arena);
...
enum _expr_kind {BoolOp_kind=1, NamedExpr_kind=2, BinOp_kind=3, PipeOp_kind=4,
                  ShellRedir_kind=5, ShellCmd_kind=6, ShellEnvAssign_kind=7,
                  UnaryOp_kind=8, Lambda_kind=9, IfExp_kind=10, Dict_kind=11,
                  ...
                  Call_kind=21, FormattedValue_kind=22, JoinedStr_kind=23,
...
struct _expr {
    ...
    union {
        ...
        struct {
            expr_ty node;
            asdl_redir_seq *redirects;
        } ShellRedir;

        struct {
            expr_ty tag;
            asdl_expr_seq *args;
            asdl_identifier_seq *space_after;
            asdl_identifier_seq *quote;
        } ShellCmd;
...
enum _redir_kind {RedirOut_kind=1, RedirIn_kind=2, RedirAppend_kind=3};
struct _redir {
    enum _redir_kind kind;
    union {
        struct {
            expr_ty file;
        } RedirOut;
...
expr_ty _PyAST_NamedExpr(expr_ty target, expr_ty value, int lineno, int
                         ...
                         PyArena *arena);
expr_ty _PyAST_BinOp(expr_ty left, operator_ty op, expr_ty right, int
                     pre_spacing, int post_spacing, int lineno, int col_offset,
                     int end_lineno, int end_col_offset, PyArena *arena);
expr_ty _PyAST_PipeOp(expr_ty left, expr_ty right, int lineno, int col_offset,
                      int end_lineno, int end_col_offset, PyArena *arena);
expr_ty _PyAST_ShellRedir(expr_ty node, asdl_redir_seq * redirects, int lineno,
                          int col_offset, int end_lineno, int end_col_offset,
                          PyArena *arena);
expr_ty _PyAST_ShellCmd(expr_ty tag, asdl_expr_seq * args, asdl_identifier_seq
                        * space_after, asdl_identifier_seq * quote, int lineno,
                        int col_offset, int end_lineno, int end_col_offset,
                        PyArena *arena);
expr_ty _PyAST_ShellEnvAssign(identifier name, expr_ty value, int lineno, int
                              col_offset, int end_lineno, int end_col_offset,
                              PyArena *arena);
...
expr_ty _PyAST_Slice(expr_ty lower, expr_ty upper, expr_ty step, int lineno,
                     int col_offset, int end_lineno, int end_col_offset,
                     PyArena *arena);
redir_ty _PyAST_RedirOut(expr_ty file, PyArena *arena);
redir_ty _PyAST_RedirIn(expr_ty file, PyArena *arena);
redir_ty _PyAST_RedirAppend(expr_ty file, PyArena *arena);
